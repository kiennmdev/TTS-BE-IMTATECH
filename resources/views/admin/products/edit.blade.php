                ...
                <div class="card">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="product-sku-input"
                            value="{{ $productEdit->sku }}" name="sku" required readonly>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="is_hot_deal" id="Hot Deal" data-choices
                            ...
                            <option value="1" {{ $productEdit->is_hot_deal == 1 ? 'selected' : '' }}>Yes
                            </option>
                            <option value="0" {{ $productEdit->is_hot_deal == 0 ? 'selected' : '' }}>No
                            </option>
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="is_new" id="New" data-choices
                            data-choices-search-false>
                            <option value="1" {{ $productEdit->is_new == 1 ? 'selected' : '' }}>Yes</option>
                            <option value="0" {{ $productEdit->is_new == 0 ? 'selected' : '' }}>No</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="is_show_home" id="Show Home" data-choices
                            data-choices-search-false>
                            <option value="1"{{ $productEdit->is_show_home == 1 ? 'selected' : '' }}>Yes
                            </option>
                            <option value="0"{{ $productEdit->is_show_home == 0 ? 'selected' : '' }}>No
                            </option>
                        </select>
                    </div>
...
<div class="modal fade bs-example-modal-xl" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel"
    ...
    <div class="modal-dialog modal-xl">
        <form class="modal-content" action="{{ route('admin.product.variants.updateMulti') }}" method="POST"
                                ...
                                <table id="alternative-pagination"
                                    ...
                                    <tbody>
                                        @foreach ($variants as $variant)
                                            @php
                                                $id = "$variant->product_color_id-$variant->product_size_id";
                                            @endphp
                                            <tr class="text-center">
                                                <td>{{ $variant->id }}</td>
                                                <td>
                                                    <div
                                                        style="width: 30px; height: 30px; border-radius:50%; background-color: {{ $variant->color->code }}">
                                                    </div>
                                                </td>
                                                <td>{{ $variant->size->name }}</td>
                                                <td><input type="number" class="form-control" value="{{ $variant->price_regular }}"
                                                        name="variant_update[{{ $variant->id }}][price_regular]">
                                                </td>
                                                <td><input type="number" class="form-control" value="{{ $variant->price_sale }}"
                                                        name="variant_update[{{ $variant->id }}][price_sale]">
                                                </td>
                                                <td><input type="number" class="form-control" value="{{ $variant->quantity }}"
                                                        name="variant_update[{{ $variant->id }}][quantity]">
                                                </td>
                                                <td>

                                                    <input type="file" id="fileInput{{ $id }}"
                                                        style="display: none;"
                                                        name="variant_update[{{ $variant->id }}][image]">
                                                    <div
                                                        style="position: relative; display: inline-block; max-width: 100px;">
                                                        <div style="padding: 5px; background-color: #E9EBEC;">
                                                            <img id="imagePreview{{ $id }}"
                                                                src="{{ Storage::url($variant->image) }}"
                                                                alt="Ảnh Sản Phẩm" style="width: 100%; height: 100%;">
                                                        </div>
                                                        <div onclick="addImageVariant('{{ $id }}')"
                                                            class="d-flex justify-content-center align-items-center"
                                                            style="width: 30px; height: 30px; color:gray; background-color: #E9EBEC; border-radius: 50%; position: absolute; top: 70%; left: 90%; cursor: pointer;">
                                                            <i class="ri-image-line"></i>
                                                        </div>
                                                    </div>
                                                </td>
                                                {{-- <td>
                                                        <button type="submit" onclick="return confirm('muon xoa ko')"
                                                            class="btn btn-sm btn-soft-danger remove-item-btn"><i
